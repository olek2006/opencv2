...
smile_cascades = cv2.CascadeClassifier("data/haarcascades/haarcascade_smile.xml")
...
while True:
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.9:
            ...
            eyes = eye_cascades.detectMultiScale(roi_gray, scaleFactor=1.1, minNeighbors= 10, minSize = (10,10))
            for (ex, ey, ew, eh) in eyes:
                 cv2.rectangle(roi_color, (ex,ey), (ex+ew,ey+eh), (0,0,255), 2)
            smile = smile_cascades.detectMultiScale(roi_gray, scaleFactor=1.7, minNeighbors=10, minSize=(25, 25))
